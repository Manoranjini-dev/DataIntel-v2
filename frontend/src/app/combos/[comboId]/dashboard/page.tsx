...
import { comboApi, dashboardApi, chatApi } from '@/lib/api';
...
export default function ComboDashboardPage() {
  ...
  async function loadData() {
    try {
      const { combo: c } = await comboApi.get(comboId);
      setCombo(c);

      // Load active chat context to preserve navigation state
      const { chats: chatList } = await chatApi.list({ comboId });
      ...
      const { dashboards } = await dashboardApi.list({ origin: 'datasource', contextType: 'combo', contextId: comboId });
  ...
  async function handleCreateDashboard() {
    ...
    setCreating(true);
    try {
      const { dashboard: newDash } = await dashboardApi.create({
  ...
  const chatUrl = activeChatId
    ? `/combos/${comboId}/chat?chatId=${activeChatId}`
    : `/combos/${comboId}/chat`;
    ...
    return (
      <div className="flex-1 flex flex-col h-full min-w-0 bg-background text-foreground animate-fade-in">
        ...
        <header className="border-b border-border bg-background/95 backdrop-blur-md px-4 py-2.5 flex items-center gap-3 shrink-0" style={{ boxShadow: 'var(--shadow-soft)' }}>
          <Link href={`/combos`} className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground text-xs mr-1 transition-colors">
