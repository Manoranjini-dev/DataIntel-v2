...
import { comboApi, chatApi, dashboardApi, cardApi } from '@/lib/api';
...
function AddToDashboardModal({ comboId, message, onClose }: {
  comboId: string; message: ComboMessage; onClose: () => void;
}) {
...
// ── Save Card Modal ─────────────────────────────────────────────
function SaveCardModal({ comboId, message, onClose }: {
  comboId: string; message: ComboMessage; onClose: () => void;
}) {
...
export default function ComboChatPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const { chats: fetchedChats } = await chatApi.list({ comboId });
      ...
      router.replace(`/combos/${comboId}/chat?chatId=${activeChatId}`, { scroll: false });
  ...
  async function loadMessages(cid: string) {
    try {
      const { messages: hist } = await chatApi.getMessages(cid);
      ...
      refreshComboMessages(cid, mapped);
    } catch (e) { console.error(e); }
  }
   ...
   */
  async function refreshComboMessages(cid: string, msgsData: ComboMessage[]) {
    ...
    setRefreshing(true);
    try {
      const { results } = await chatApi.refreshComboMessages(cid, toRefresh);
  ...
  async function switchChat(cid: string) {
    ...
    setMessages([]);
    setShowChatList(false);
    router.replace(`/combos/${comboId}/chat?chatId=${cid}`, { scroll: false });
  ...
  async function newChat() {
    ...
    setMessages([]);
    setShowChatList(false);
    router.replace(`/combos/${comboId}/chat?chatId=new`, { scroll: false });
  }
  ...
  async function handleSend(e?: React.FormEvent) {
    ...
    try {
      ...
      if (activeCid === 'new') {
        ...
        const { chat } = await chatApi.create({ comboId, title });
        activeCid = chat.id;
        setCurrentChatId(activeCid);
        setChats(prev => [chat, ...prev]);
        router.replace(`/combos/${comboId}/chat?chatId=${activeCid}`, { scroll: false });
      }

      // Pass chatId so the backend persists user + assistant messages
      const result = await comboApi.query(comboId, prompt, activeCid);
      ...
      if (currentChat && (!currentChat.title || currentChat.title === 'New Combo Chat' || currentChat.title === combo?.name)) {
        ...
        await chatApi.updateTitle(activeCid, autoTitle).catch(() => {});
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      {/* ── Modals ───────────────────────────────────────────── */}
      {addToDashMsg && (
        <AddToDashboardModal comboId={comboId} message={addToDashMsg} onClose={() => setAddToDashMsg(null)} />
      )}
      {saveCardMsg && (
        <SaveCardModal comboId={comboId} message={saveCardMsg} onClose={() => setSaveCardMsg(null)} />
      )}
    </div>
  );
}
