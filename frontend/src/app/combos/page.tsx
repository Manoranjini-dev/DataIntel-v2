...
import { comboApi, connectionApi } from '@/lib/api';
...
export default function CombosPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    e.preventDefault();
    if (form.connectionIds.length < 2) return;
    setSubmitting(true);
    try {
      const { combo } = await comboApi.create(form);
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                ...
                <input value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))}
                  ...
                  className="w-full px-3 py-2.5 bg-muted/50 border border-border rounded-xl text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/40"/>
              </div>
              <div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button type="button" onClick={() => setShowCreate(false)}
                  className="px-4 py-2 bg-muted/50 hover:bg-muted rounded-xl text-sm text-muted-foreground transition-colors">
                  Cancel
                </button>
              </div>
            </form>
        ...
        <div className="space-y-3">
          {combos.length === 0 ? (
            ...
            combos.map((combo: any) => (
              <div key={combo.id}
                className="flex items-start gap-4 px-5 py-4 bg-card border border-border hover:border-primary/30 rounded-2xl transition-all group">
                ...
                <div className="flex gap-2">
                  <Link href={`/combos/${combo.id}/chat?chatId=new`}
                  ...
                  <Link href={`/chats?comboId=${combo.id}`}
                    className="px-3 py-1.5 bg-muted/50 border border-border hover:bg-muted rounded-lg text-xs text-muted-foreground transition-colors">
                    History
                  </Link>
                  <Link href={`/combos/${combo.id}/dashboard`}
                    className="px-3 py-1.5 bg-muted/50 border border-border hover:bg-muted rounded-lg text-xs text-muted-foreground transition-colors">
                    Dashboard
                  </Link>
                </div>
