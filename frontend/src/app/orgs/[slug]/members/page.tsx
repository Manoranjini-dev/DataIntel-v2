...
const ROLE_COLORS: Record<string, string> = {
  ...
  member: 'bg-zinc-500/10 border-zinc-500/20 text-muted-foreground',
};
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">{org?.name} · {members.length} member{members.length !== 1 ? 's' : ''}</p>
          </div>
        </div>
        ...
        {isOwnerOrAdmin && (
          <div className="bg-muted/30 border border-border rounded-2xl p-5 mb-6">
            <h2 className="text-sm font-medium text-foreground mb-3">Invite Member</h2>
            <form onSubmit={handleInvite} className="flex gap-3">
              <input
                ...
                className="flex-1 px-3 py-2.5 bg-muted/50 border border-border rounded-xl text-sm text-white placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/40"
              />
              ...
              <button type="submit" disabled={!inviteEmail.trim() || inviting}
                className="px-4 py-2.5 bg-primary hover:opacity-90 rounded-xl text-sm font-medium disabled:opacity-40 transition-colors whitespace-nowrap">
        ...
        <div className="space-y-2">
          {members.map((member: any) => (
            <div key={member.account_id || member.id}
              className="flex items-center gap-4 px-4 py-3 bg-muted/30 border border-white/[0.06] rounded-xl">
              <div className="w-9 h-9 rounded-full bg-primary/20 border border-primary/20 flex items-center justify-center text-sm font-bold text-primary flex-shrink-0">
