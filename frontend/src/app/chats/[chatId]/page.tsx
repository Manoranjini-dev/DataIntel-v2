...
import { chatApi, dashboardApi, cardApi } from '@/lib/api';
...
export default function ChatPage() {
  ...
  async function loadNewChatSetup() {
    try {
      ...
      const { chats: chatList } = await chatApi.list(chatFilter);
  ...
  async function loadData() {
    try {
      ...
      const { messages: msgs } = await chatApi.getMessages(chatId);
  ...
  async function handleSend(e?: React.FormEvent) {
    ...
    try {
      ...
      if (chatId === 'new') {
        const { chat: newChat } = await chatApi.create({ 
      ...
      if (newChatCreated) {
        ...
        router.replace(`/chats/${targetChatId}?connectionId=${connectionId || ''}`);
        return; 
      }
  ...
  async function handleExecuteDraft(executionId: string, sql: string) {
    if (!chat) return;
    try {
      const result = await chatApi.executeDraft(chat.id, executionId, sql);
  ...
  async function handleAddToDashboard(message: MessageBubble) {
    if (!chat) return;
    try {
      const { dashboards } = await dashboardApi.list({ origin: 'datasource', contextType: 'connection', contextId: chat.connection_id });
      let targetDash = dashboards.find((d: any) => d.connection_id === chat.connection_id);
      if (!targetDash) {
        const res = await dashboardApi.create({ name: 'Main Dashboard', connectionId: chat.connection_id, origin: 'datasource' });
        targetDash = res.dashboard;
      }
      
      const { pages } = await dashboardApi.get(targetDash.id);
      let pageId = pages?.[0]?.id;
      if (!pageId) {
        const { page } = await dashboardApi.addPage(targetDash.id, 'Page 1');
        pageId = page.id;
      }

      await dashboardApi.addWidget(targetDash.id, pageId, {
  ...
  async function handleSaveAsCard(message: MessageBubble) {
    ...
    try {
      ...
      const { card } = await cardApi.create({
        ...
        queryDefinition: { sql: message.generated_query || `-- Natural language query:\n-- ${message.content}` },
        ...
        <div className="px-4 py-3 border-b border-border flex-shrink-0 space-y-3">
          ...
          {connectionId && (
            <div className="flex flex-col gap-1.5 pt-2 border-t border-white/5">
              <Link href={`/dashboards?connectionId=${connectionId}`} className="flex items-center gap-2.5 text-xs text-muted-foreground hover:text-foreground transition-colors py-1.5 px-2 rounded-lg hover:bg-muted/50">
                <LayoutDashboard className="w-4 h-4 text-amber-500 shrink-0" /> Dashboards
              </Link>
              <Link href={`/connections/${connectionId}/schema`} className="flex items-center gap-2.5 text-xs text-muted-foreground hover:text-foreground transition-colors py-1.5 px-2 rounded-lg hover:bg-muted/50">
          ...
          {allChats.length === 0 ? (
            ...
            <div className="space-y-0.5 px-2">
              {allChats.map((c: any) => (
                <Link
                  key={c.id}
                  href={`/chats/${c.id}${connectionId ? `?connectionId=${connectionId}` : ''}`}
