...
export default function ERDPage() {
  ...
  async function loadData() {
    try {
      ...
      const tablesRes = await fetch(`${API}/connections/${connId}/schema/tables`, { credentials: 'include' });
      ...
      const colResults = await Promise.allSettled(
        tableList.map(async (t: any) => {
          const r = await fetch(`${API}/connections/${connId}/schema/tables/${encodeURIComponent(String(t.table_name))}/columns`, { credentials: 'include' });
  ...
  if (error) return (
    <div className="h-full flex items-center justify-center">
      <div className="bg-muted/50 border border-border rounded-2xl p-8 max-w-md text-center shadow-lg">
        ...
        <Link href={`/connections/${connId}`} className="px-4 py-2 bg-primary hover:opacity-90 rounded-lg text-sm font-medium transition-colors">
  ...
  return (
    <div className="h-full w-full flex flex-col overflow-hidden relative">
      <header className="absolute top-0 left-0 right-0 z-10 px-6 py-4 flex items-center justify-between pointer-events-none">
        <Link href={`/connections/${connId}/schema`} className="pointer-events-auto flex items-center gap-2 px-4 py-2 bg-card/90 border border-border rounded-xl text-sm font-medium hover:bg-muted backdrop-blur-md transition-all text-foreground shadow-xl">
