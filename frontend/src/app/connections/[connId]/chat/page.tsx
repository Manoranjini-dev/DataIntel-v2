...
import { chatApi, connectionApi, dashboardApi, cardApi } from '@/lib/api';
...
/**
 * Map a thrown request error (APIError, axios timeout, network failure, or an
 * unexpected throw) to a single clear, user-facing sentence. Guarantees a
 * non-empty message so the UI never shows a bare "Error:".
 */
function formatChatError(err: any): string {
  // Axios / network-level failures never reach handleResponse, so they have no
  // structured body — detect them by code/message shape.
  const code = err?.code || err?.cause?.code;
  const rawMsg: string = err?.message || '';
  const lower = rawMsg.toLowerCase();

  if (code === 'ECONNABORTED' || lower.includes('timeout')) {
    return 'The request timed out. The question may be too complex, or the service is busy — please try again.';
  }
  ...
  // APIError carries an HTTP status + structured backend message.
  const status: number | undefined = err?.status ?? err?.structured?.status;
  const backendMsg: string = (err?.structured?.message || rawMsg || '').trim();

  if (status === 401) return 'Your session has expired. Please sign in again.';
  if (status === 403) return 'You do not have permission to query this connection.';
  if (status === 404) return 'This chat or connection could not be found. Try starting a new chat.';
  if (status === 400 && backendMsg) return backendMsg; // validation messages are already user-friendly
  if (status && status >= 500) {
    return backendMsg && !/^http\s/i.test(backendMsg)
      ? backendMsg
      : 'The server ran into a problem generating your answer. Please try again in a moment.';
  }

  // Fallback — always non-empty.
  return backendMsg || 'Something went wrong while answering your question. Please try again.';
}
        ...
        >
          {message.isError ? (
            <div className="flex items-start gap-2 text-sm text-destructive">
              ...
              <span className="leading-relaxed">{message.content || 'Something went wrong. Please try again.'}</span>
            </div>
          ) : (
            <div className="text-sm text-foreground leading-relaxed prose prose-sm max-w-none prose-p:my-1 prose-strong:text-foreground prose-code:text-primary prose-code:bg-muted prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-code:text-xs">
              <ReactMarkdown remarkPlugins={[remarkGfm]}>
                {message.content?.trim() || '_No response was returned. Please try rephrasing your question._'}
              </ReactMarkdown>
            </div>
          )}
...
export default function ConnectionChatPage() {
  ...
  async function loadData() {
    try {
      ...
      setConn(c);
      const { chats: chatList } = await chatApi.list({ connectionId: connId });
      setChats(chatList);
      if (currentChatId && currentChatId !== 'new') await loadMessages(currentChatId);
  ...
  async function loadMessages(cid: string) {
    try {
      const { messages: msgs } = await chatApi.getMessages(cid);
  ...
  async function refreshMessages(cid: string, msgsData: Message[]) {
    ...
    setRefreshing(true);
    try {
      const { results } = await chatApi.refreshMessages(cid, toRefresh);
  ...
  async function switchChat(cid: string) {
    ...
    router.replace(`/connections/${connId}/chat?chatId=${cid}`);
  }
  ...
  function newChat() {
    ...
    setMessages([]);
    setShowChatList(false);
    router.replace(`/connections/${connId}/chat?chatId=new`);
  }
  ...
  async function handleRunSQL(messageId: string, executionId: string, sql: string) {
    ...
    try {
      const result = await chatApi.executeDraft(currentChatId, executionId, sql);
      ...
      console.error('[chat] run SQL failed:', err);
      ...
      setMessages(ms => ms.map(m => m.id === messageId ? {
        ...
        pending_execution: false,
        sql_running: false,
        // Surface the failure as a clear, separate line rather than appending a
        // bare "Error:" onto the existing answer text.
        content: `${m.content}\n\n**Could not run the query:** ${friendly}`,
      } : m));
    }
  }
  ...
  async function handleSend(e?: React.FormEvent) {
    ...
    try {
      ...
      if (!targetChatId) {
        const { chat: newChatObj } = await chatApi.create({
          connectionId: connId,
        ...
        router.replace(`/connections/${connId}/chat?chatId=${targetChatId}`);
      }

      // Pass autoExecute preference to backend
      const result = await chatApi.ask(targetChatId!, prompt, autoExecute);
      ...
      const friendly = formatChatError(err);
      // Keep the user's prompt visible (drop only the temp placeholder) and
      // append a clearly-styled error bubble with a meaningful message.
      setMessages(ms => [
...
// ── Add to Dashboard Modal ─────────────────────────────────────
function AddToDashboardModal({ connId, message, onClose }: {
  connId: string; message: Message; onClose: () => void;
}) {
  ...
  useEffect(() => {
    ...
    dashboardApi.get(selectedDash).then(res => {
      setPages(res.pages || []);
  ...
  async function handleAdd() {
    ...
    try {
      ...
      await dashboardApi.addWidget(selectedDash, selectedPage, {
...
// ── Save Card Modal ────────────────────────────────────────────
function SaveCardModal({ connId, message, onClose }: {
  connId: string; message: Message; onClose: () => void;
}) {
    ...
    try {
      ...
      await cardApi.create({
        ...
        queryDefinition: {
          ...
          sql:       message.generated_query || `-- Natural language query:\n-- ${message.content}`,
          ui_hint:   chartType,
        },
      });
      setDone(true);
