...
import { connectionApi, chatApi } from '@/lib/api';
...
export default function SchemaExplorerPage() {
  ...
  async function loadTables(q: string) {
    try {
      const qs = q ? `?q=${encodeURIComponent(q)}` : '';
      const r = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api'}/connections/${connId}/schema/tables${qs}`,
        { credentials: 'include' }
      );
  ...
  async function handleExplainDB() {
    ...
    try {
      ...
      const { chats } = await chatApi.list({ connectionId: connId });
      let chatId: string;
      if (chats.length > 0) { chatId = chats[0].id; }
      else { const { chat } = await chatApi.create({ connectionId: connId }); chatId = chat.id; }
      const result = await chatApi.ask(chatId,
  ...
  async function handlePreviewData(tableName: string) {
    ...
    try {
      ...
      const result = await chatApi.ask(chatId, `SELECT * FROM ${tableName} LIMIT 10`, true);
  ...
  async function loadColumns(tableName: string) {
    ...
    try {
      const r = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api'}/connections/${connId}/schema/tables/${encodeURIComponent(tableName)}/columns`,
        { credentials: 'include' }
      );
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <header className="border-b border-border px-6 py-3 flex items-center justify-between flex-shrink-0 bg-card/60">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href={`/connections/${connId}/erd`} className="px-3 py-1.5 bg-primary hover:opacity-90 rounded-lg text-xs font-medium text-white transition-colors flex items-center gap-1.5">
            ...
            <div className="p-8 max-w-6xl mx-auto">
              ...
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Link href={`/connections/${connId}/chat`} className="px-3 py-1.5 bg-primary hover:opacity-90 rounded-lg text-xs font-medium text-white transition-colors flex items-center gap-1.5">
