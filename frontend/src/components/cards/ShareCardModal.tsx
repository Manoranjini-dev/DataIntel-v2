...
export function ShareCardModal({ cardId, cardName, onClose }: ShareCardModalProps) {
  ...
  async function loadShares() {
    setLoading(true);
    try {
      const { shares } = await cardApi.listShares(cardId);
      setShares(shares);
  ...
  async function handleRevoke(accountId: string) {
    try {
      await cardApi.revokeShare(cardId, accountId);
      await loadShares();
    } catch (e) { console.error(e); }
  }
  ...
  async function handleAccessChange(accountId: string, level: 'view' | 'edit') {
    try {
      await cardApi.updateShare(cardId, accountId, { accessLevel: level });
      await loadShares();
    } catch (e) { console.error(e); }
  }
      ...
      <div className="bg-card w-full max-w-md rounded-xl shadow-lg border border-border overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b border-border">
          <h2 className="text-lg font-semibold flex items-center gap-2">
            <Share2 className="w-5 h-5 text-primary" /> Share Card
          </h2>
