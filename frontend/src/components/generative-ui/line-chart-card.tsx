...
export function LineChartCard({ execution, title, compact, showLegend = true }: LineChartCardProps) {
  ...
  // Axis titles derive from the plotted fields (X = category/date, Y = the
  // single measure). With multiple series the legend names them, so we omit a
  // single Y title to avoid mislabeling.
      ...
      <div className={`w-full ${compact ? 'flex-1 min-h-0 overflow-auto' : ''}`}>
        <div style={{ minWidth: compact ? calculatedWidth : undefined, height: compact ? "100%" : 220 + xAxisHeight - 30, width: '100%' }}>
          <ResponsiveContainer width="99%" height="100%">
            <LineChart data={schema.data} margin={{ top: 10, right: 20, left: yTitle ? Y_TITLE_SPACE : 10, bottom: 0 }}>
