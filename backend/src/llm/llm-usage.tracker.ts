...
export class LLMUsageTracker {
  ...
  async trackUsage(
    ...
    try {
      ...
      await this.redis.getClient().hincrbyfloat(redisKey, 'costUsd', costUsd);
      await this.redis.getClient().hincrby(redisKey, 'promptTokens', promptTokens);
      await this.redis.getClient().hincrby(redisKey, 'completionTokens', completionTokens);
      await this.redis.getClient().hincrby(redisKey, 'requestCount', 1);
  ...
  async getUsageStats(orgId: string, monthKey: string) {
    const redisKey = `di:llm_usage:${orgId}:${monthKey}`;
    const stats = await this.redis.getClient().hgetall(redisKey);
